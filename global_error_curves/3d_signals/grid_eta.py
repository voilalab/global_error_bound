  ...
  for index in range(len(IDX_LIST)):
    ...
    for grid_reso in grid_resos:
        ...
        while abs(max_dist - prev_max_dist) > tol:
            output = fit_grid(target_signal=signal, r = grid_reso, iters = iters, lr=learning_rate, interp=args.interp, log_interval=iters+1, seed=args.seed + i * grid_reso, device=device)
