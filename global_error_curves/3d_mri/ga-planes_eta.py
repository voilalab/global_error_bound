    ...
    if args.param_to_run == 'volume_resolution':
      ...
      for volume_reso in volume_resos:
        ...
        while abs(max_dist - prev_max_dist) > tol:
            output = fit_gaplanes(args=gap_args, img=signal, iters=iters, learning_rate=learning_rate, log_interval = iters+1, seed=args.seed + i * volume_reso, device=device, mask=mask)
      ...
      for feature_dim in feature_dims:
        ...
        eta, list_of_outputs, i = None, [], 0
        while abs(max_dist - prev_max_dist) > tol:
            output = fit_gaplanes(args=gap_args, img=signal, iters=iters, learning_rate=learning_rate, log_interval = iters+1, seed=args.seed + i * feature_dim, device=device, mask=mask)
            error = np.linalg.norm(signal.flatten() - output['best_pred'].flatten())
            list_of_outputs.append(output['best_pred'].flatten())
            if len(list_of_outputs) >= 2:
                prev_max_dist = max_dist
