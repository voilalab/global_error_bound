...
PARAMS_DICT = {
    'mapping_size': {
        'mapping_sizes': [15, 31, 48, 64, 80, 97, 113, 130, 146, 162, 179, 195, 211, 228, 244, 261, 277, 293, 310, 326, 343, 359, 375, 392, 408, 424, 441, 457, 474, 1520],
        'num_layers': 1,
        'width': 400,
        'threshold': 0,
    },
    'width': {
        'widths': [4, 8, 12, 17, 21, 25, 30, 34, 38, 43, 47, 51, 56, 60, 64, 68, 73, 77, 81, 86, 90, 94, 99, 103, 107, 112, 116, 120, 124, 400],
        'mapping_size': 1520,
        'num_layers': 1,
        'threshold': 0,
    },
  ...
  for index in range(len(IDX_LIST)):
    ...
    if args.param_to_run == 'mapping_size':
      ...
      with open(f"pkls/{MODEL_NAME}/{args.param_to_run}_i{idx}_s{args.seed}.pkl", "wb") as f:
        ...
        # 1. Configure the logger
        logging.basicConfig(
            filename=f"../results/3d_mri/{MODEL_NAME}.pkl",  # Name of the log file
            level=logging.INFO,  # Minimum log level to capture
            format='%(message)s', # Log message format
            filemode='w', # Overwrite the file each time the script runs (use 'a' to append)
            force=True,
