...
def run_exp(args):
  ...
  for index in range(len(IDX_LIST)):
    ...
    if args.param_to_run == 'num_levels':
      ...
      logging.basicConfig(
        filename=f"logs/{MODEL_NAME}/{args.param_to_run}_i{idx}_s{args.seed}.log",  # Name of the log file
      ...
      for num_levels in nums_levels:
        ...
        while abs(max_dist - prev_max_dist) > tol:
            ...
            output = fit_instant_ngp(signal, model_config, iters=iters, learning_rate=learning_rate, log_interval = iters+1, seed=args.seed + i * num_levels, device=device, mask=mask)
      ...
      with open(f"pkls/{MODEL_NAME}/{args.param_to_run}_i{idx}_s{args.seed}.pkl", "wb") as f:
        ...
        for hash_size in hash_sizes:
          ...
          while abs(max_dist - prev_max_dist) > tol:
              ...
              output = fit_instant_ngp(signal, model_config, iters=iters, learning_rate=learning_rate, log_interval = iters+1, seed=args.seed + i * hash_size, device=device, mask=mask)
              error = np.linalg.norm(signal.flatten() - output['best_pred'].flatten())
              list_of_outputs.append(output['best_pred'].flatten())
              if len(list_of_outputs) >= 2:
                  prev_max_dist = max_dist
